...
def ADMM_objective(m, arguments, agent_intersection, neighbouring_intersections, it, diff, norm, x, lambda_, z_g, Aditional_obj):
    """
    ##################### ADMM TERMS #####################
    """
    ...
    for neighbour in neighbouring_intersections:
        ...
        Aditional_obj.add( -lambda_[(it, agent_intersection)][neighbour].T @ x.select(neighbour, '*'))
        
        
        # Difference between x and z
        m.addConstrs((
                    diff[neighbour, phase] == x[neighbour, phase] - z_g[(it, neighbour)][phase]
        ...
        Aditional_obj -= norm[neighbour]*norm[neighbour]*arguments["params"]["rho"]/2
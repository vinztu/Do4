...
def optimizer(sim, model, opt_vars):
    ...
    try:
        ...
        for intersection in sim.intersections_data:
            ...
            opt_phase = [round(opt_vars["phi"][0, intersection, phase].X) for phase in sim.params["phases"]].index(1)
...
def calculate_objective_per_intersection(sim, opt_vars, intersection):
    ...
    for tau in range(sim.params["prediction_horizon"] + 1):
        ...
        opt_phase = [round(opt_vars["phi"][tl_update, intersection, phase].X) for phase in sim.params["phases"]].index(1)
        
        # determine the corresponding pressure for that phase
        obj_per_int += (sim.params["alpha"] ** tau) * opt_vars["p_p"][tau, intersection, opt_phase]

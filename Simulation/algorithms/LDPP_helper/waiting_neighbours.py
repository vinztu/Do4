...
def waiting_x_and_lambda_neighbours(arguments, it, intersection, global_state):
    ...
    for neighbour in arguments["intersections_data"][intersection]["neighbours"]:
        ...
        while True:
            ...
            if previous_x_it_decision != None:
                break
        ...
        if arguments["params"]["z_domain"] == "binary":
            while True:
                previous_lambda_it_decision = ray.get(global_state.get_lambda_.remote(it, neighbour))
                if previous_lambda_it_decision != None:
                    break
            
            neighbours_lambda[(it, neighbour)] = previous_lambda_it_decision
...
def waiting_z_g_neighbours(arguments, it, intersection, global_state):
    ...
    for neighbour in arguments["intersections_data"][intersection]["neighbours"]:
        # get x from global state
        while True:
            previous_z_g_it_decision = ray.get(global_state.get_z.remote(it, neighbour))
            if previous_z_g_it_decision != None:
                break
